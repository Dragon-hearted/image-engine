/**
 * Vindof Mirror Walk v2 Storyboard Generator
 *
 * 12-scene rewrite with corrected walking directions:
 *   Character A moves left-to-right throughout (enters L edge, exits R edge)
 *   Character B moves right-to-left throughout (enters R edge, exits L edge)
 *   Realization scenes (old S08, old S12) removed.
 *
 * Uses NanoBanana Pro (gemini-3-pro-image-preview) at 2K. Scene references
 * chain for continuity.
 *
 * Run from image-engine directory:
 *   cd systems/image-engine && bun scripts/generate-vindof-mirror-walk-v2.ts
 */
...
// ─── Scene 01: Convergence Wide (0.0-1.8s) ───

const s01 = await runScene({
	sceneNum: "01",
	sceneName: "Convergence Wide",
	assetBasename: "v2-scene-01-convergence-wide",
...
No text in image.`,
	referenceImageIds: ["vindof-lazy-bunch-shirt", "vindof-red-dot-polo"],
});

// ─── Scene 02: The Bump — slow-mo (1.8-3.1s) ───

const s02 = await runScene({
	sceneNum: "02",
	sceneName: "The Bump (slow-mo)",
	assetBasename: "v2-scene-02-bump",
	systemInstruction: `Cinematic medium two-shot capturing a slow-motion shoulder bump. Overcast diffused daylight. Two Indian Gen Z males at the instant of impact at the EXACT center of frame — A moving screen-right, B moving screen-left, shoulders clipping. Four AirPods caught mid-air. Slight handheld micro-jitter. Shallow depth of field. Photorealistic, filmic grain.`,
	prompt: `${PREAMBLE}

Scene context: The impact moment — continuation of Scene 01. Character A (still moving LEFT-to-RIGHT) and Character B (still moving RIGHT-to-LEFT) have just closed the distance and their shoulders clip at the exact center of frame. Four white AirPods caught in mid-air arcing away from their ears as a result of the bump. Hair shifting from the impact. Slow-motion feel.

Subject: Medium two-shot from knees to head. ${CHAR_A} He is on the LEFT half of frame, his body in 3/4 profile facing screen-right, right shoulder leading into contact at the vertical center. ${CHAR_B} He is on the RIGHT half of frame, his body in 3/4 profile facing screen-left (mirror to A), left shoulder leading into contact at the vertical center. Their two shoulders clip at the exact center vertical line. Four white AirPods Pro earbuds caught mid-air around their heads, trajectories radiating outward from the collision point — two AirPods arcing up and toward camera-left from Character A's ear area, two AirPods arcing up and toward camera-right from Character B's ear area. Both characters' hair lifting slightly from the impact.

Environment: Same museum plaza as Scene 01. Polished pale stone floor under their feet. Glass + brushed steel facade behind them, minimal mullions, reflecting the overcast sky. Plaza still empty.
...
Lighting: Same overcast wrap light as Scene 01, consistent across the sequence. No hard shadows. Soft sheen on the stone floor and a subtle highlight roll on the glass facade behind.
...
// ─── Scene 03: AirPods Fall — overhead (3.1-3.8s) ───

const s03 = await runScene({
	sceneNum: "03",
	sceneName: "AirPods Fall (overhead)",
	assetBasename: "v2-scene-03-airpods-fall",
...
No text in image.`,
	referenceImageIds: [s01],
});

// ─── Scene 04: Pickup Swap (3.8-4.6s) ───

const s04 = await runScene({
	sceneNum: "04",
	sceneName: "Pickup Swap",
	assetBasename: "v2-scene-04-pickup-swap",
...
Scene context: Top-down overhead continuation of Scene 03. Two hands enter the frame diagonally from opposite edges, each reaching to grab a pair of AirPods — but each hand is grabbing the wrong pair. This is the swap moment.
...
No text in image.`,
	referenceImageIds: [s03, "vindof-lazy-bunch-shirt", "vindof-red-dot-polo"],
});

// ─── Scene 05: Character A Inserts Wrong Pair (4.6-5.3s) ───

const s05 = await runScene({
	sceneNum: "05",
	sceneName: "Character A Inserts",
	assetBasename: "v2-scene-05-a-inserts",
...
No text in image.`,
	referenceImageIds: [s01, "vindof-lazy-bunch-shirt"],
});

// ─── Scene 06: Beat Drop → B Fit Detail 1 (5.3-6.0s) ───

const s06 = await runScene({
	sceneNum: "06",
	sceneName: "Beat Drop → B Fit Detail 1",
	assetBasename: "v2-scene-06-b-detail-1",
...
No text in image.`,
	referenceImageIds: ["vindof-red-dot-polo", s01],
});

// ─── Scene 07: B Fit Detail 2 (6.0-6.7s) ───

const s07 = await runScene({
	sceneNum: "07",
	sceneName: "B Fit Detail 2",
	assetBasename: "v2-scene-07-b-detail-2",
...
No text in image.`,
	referenceImageIds: ["vindof-red-dot-polo", s06],
});

// ─── Scene 08: Character B Inserts Wrong Pair (6.7-7.4s) ───
...
// ─── Scene 09: Beat Drop → A Fit Detail 1 (7.4-8.1s) ───
...
// ─── Scene 10: A Fit Detail 2 (8.1-8.8s) ───
...
// ─── Scene 11: Handoff — hands (8.8-11.0s) ───
...
console.log("\n═══════════════════════════════════════════════");
console.log("  GENERATION COMPLETE (v2)");
console.log("═══════════════════════════════════════════════\n");
