...
| **Multi-provider model routing** | One endpoint, three providers. Higgsfield (higgsfield-nano-banana-pro — **the default**) shells out to the local `higgsfield` CLI; the default CLI model is **NanoBanana Pro** (`job_set_type` `nano_banana_2`), with `higgsfield-gpt-image-2` (→ `gpt_image_2`) also selectable, and `HIGGSFIELD_MODEL` overriding the CLI token globally. Gemini models (gemini-3-pro-image-preview, gemini-3.1-flash-image-preview, gemini-2.5-flash-image) call WisGate's generateContent API; OpenAI models (gpt-image-2, gpt-image-1.5) route to WisGate's OpenAI-compatible /v1/images/generations. Selection is automatic: omit `model` (or pass one starting with 'higgsfield') for Higgsfield, names starting with 'gpt-' use the OpenAI path, everything else is Gemini. Gemini/OpenAI share one WISDOM_GATE_KEY. **Any caller that omits `model` now gets Higgsfield/NanoBanana Pro.** Higgsfield reports no token accounting, so its generations record zero token usage. |
| **Permission-gated fallback (no silent provider swap)** | If a Higgsfield generation fails (unauth/timeout/CLI/no-URL), ImageEngine does **not** silently swap providers. By default it surfaces a clear, actionable error (fix the CLI with `higgsfield auth login`, retry with an explicit `model`, or permit a fallback). An automatic fallback to `gemini-2.5-flash-image` occurs **only** when explicitly permitted: per-request via `autoFallback: true`, or globally via `IMAGE_ENGINE_AUTO_FALLBACK=1`. When a permitted fallback runs, the gallery/ledger record the provider actually served. |
...
- **Higgsfield CLI (HOST prerequisite for the default provider)** — `npm install -g @higgsfield/cli`, then authenticate once with `higgsfield auth login`. The default model `higgsfield-nano-banana-pro` (CLI `job_set_type` `nano_banana_2`) shells out to this binary; without it (or its auth) Higgsfield generations fail with a clear actionable error rather than silently swapping providers (see the permission-gated fallback below). Override the binary path with `HIGGSFIELD_BIN`, or the CLI model with `HIGGSFIELD_MODEL`, if needed.
- A WisGate (JuheAPI) account key set as WISDOM_GATE_KEY in systems/image-engine/.env (re-read from .env on every call, so rotating it needs no restart) — required for the Gemini/OpenAI models and the (opt-in) Higgsfield → gemini fallback
- For real Gemini/OpenAI generation (and the opt-in Higgsfield fallback): a positive WisGate dollar balance — when the balance is exhausted the budget guard hard-stops generation with HTTP 402
- Optional env to permit auto-fallback when Higgsfield fails: `IMAGE_ENGINE_AUTO_FALLBACK=1` (or per-request `autoFallback: true`). Default is no auto-fallback.
...
> **Expected:** HTTP 201 GenerationResult {id, imageUrl:'/api/gallery/<id>/image', model, prompt, tokenUsage, createdAt}. With no `model`, ImageEngine routes to Higgsfield/NanoBanana Pro (`higgsfield-nano-banana-pro` → CLI `nano_banana_2`) via the local `higgsfield` CLI; aspectRatio maps to the Higgsfield enum (unsupported ratios collapse to 16:9). `tokenUsage` is zero (the CLI reports none). If the CLI is missing/unauthenticated or the job fails, the request returns HTTP 500 with a clear actionable error (no silent provider swap) — unless auto-fallback is permitted via `autoFallback: true` or `IMAGE_ENGINE_AUTO_FALLBACK=1`, in which case it falls back to `gemini-2.5-flash-image` and `model` in the result reflects the served provider. Requires the `higgsfield` CLI authenticated on the host.
